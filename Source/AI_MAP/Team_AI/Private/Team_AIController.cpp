...
ATeam_AIController::ATeam_AIController()
{
	...
	*/
	...
	SenseSight->PeripheralVisionAngleDegrees = 60.0f; // 주변 시야각
	SenseSight->DetectionByAffiliation.bDetectEnemies = true; // 소속별 탐지 적
	SenseSight->DetectionByAffiliation.bDetectFriendlies = true; // 소속별 탐지 팀
	SenseSight->DetectionByAffiliation.bDetectNeutrals = true; // 소속별 탐지 중립
...
void ATeam_AIController::OnAIMoveComplete(FAIRequestID RequestID, EPathFollowingResult::Type Result)
{
	...
	if (!AICharacter)
		return;
	// 이동 완료 시 Behavior Tree의 상태를 업데이트
